...
namespace Transsevisgroup
{
    public partial class RegisterForm : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SQLiteConnection conn = Database.GetConnection()) // ✅ SQLite подключение
                {
                    conn.Open();

                    // Проверка на уникальность логина
                    SQLiteCommand checkCmd = new SQLiteCommand("SELECT COUNT(*) FROM Users WHERE Login = @login", conn);
                    checkCmd.Parameters.AddWithValue("@login", login);

                    long count = (long)checkCmd.ExecuteScalar(); // ✅ SQLite возвращает long
                    if (count > 0)
                    {
