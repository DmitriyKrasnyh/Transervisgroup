...
namespace Transsevisgroup
{
    public partial class BookingsAdminForm : Form
    {
        ...
        private void LoadServices()
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT Id, Название FROM ServiceItems", conn);
        ...
        private void LoadUsers()
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT Id, FullName FROM Users", conn);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                dt.Load(cmd.ExecuteReader());
            ...
            using (var conn = Database.GetConnection())
            {
                ...
                string query = @"
                    ...
                    WHERE (@serviceId = 0 OR b.УслугаId = @serviceId)
                      ...
                      AND (CAST(b.Время AS DATE) = @date OR @date IS NULL)
                ";

                SqlCommand cmd = new SqlCommand(query, conn);

                int serviceId = (int)comboServiceFilter.SelectedValue;
                int userId = (int)comboUserFilter.SelectedValue;
                DateTime selectedDate = dateFilter.Checked ? dateFilter.Value.Date : DateTime.MinValue;

                cmd.Parameters.AddWithValue("@serviceId", serviceId);
                cmd.Parameters.AddWithValue("@userId", userId);
                cmd.Parameters.AddWithValue("@date", dateFilter.Checked ? selectedDate : (object)DBNull.Value);
