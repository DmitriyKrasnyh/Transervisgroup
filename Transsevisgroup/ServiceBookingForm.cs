    ...
    public partial class ServiceBookingForm : Form
    {
        ...
        private void LoadServices()
        {
            using (SqlConnection conn = Database.GetConnection())
            {
                conn.Open();
                string query = @"
            SELECT si.Id, si.Название + ' (' + ISNULL(CONVERT(varchar, sp.Цена), 'без цены') + '₽)' AS DisplayName
        ...
        private bool IsTimeSlotTaken(DateTime dateTime)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM Bookings WHERE [Время] = @datetime", conn);
        ...
        private int GetClientTypeId()
        {
            using (SqlConnection conn = Database.GetConnection())
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM Bookings WHERE ПользовательId = @userId", conn);
                cmd.Parameters.AddWithValue("@userId", userId);
                int count = (int)cmd.ExecuteScalar();
